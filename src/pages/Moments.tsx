...
import type { Moment, MomentsResponse } from '@/types';
...
export default function Moments() {
  ...
  const reportViews = useCallback(async () => {
    ...
    try {
      ...
      if (res.success && Array.isArray(res.data)) {
        const updates = (res.data as Array<{ id: number; views: number }>).filter((item): item is { id: number; views: number } => (
          typeof item === 'object' &&
          item !== null &&
          'id' in item &&
          'views' in item &&
        ...
        const updatedViewsMap = new Map(updates.map((item: { id: number; views: number }) => [item.id, item.views]));
  ...
  const momentViewModels = useMemo(() => (
    moments.map((moment, index, list) => {
      const createdAt = moment.created_at || new Date().toISOString();
      const { month, day, year } = getTimelineDate(createdAt);
      const prevCreatedAt = list[index - 1]?.created_at || '';
      const isNewYear = index === 0 || (prevCreatedAt && new Date(prevCreatedAt).getFullYear() !== year);
      return {
        moment,
        month,
        day,
